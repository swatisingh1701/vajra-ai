...
async function analyzeThreat() {
  ...
  if (message === "") {
    ...
    recommendationText.innerHTML = "Paste a message and begin analysis.";
    return;
  }
  ...
  try {
    ...
    if (!data.choices) {
      ...
      indicatorList.innerHTML = "Groq API returned an error.";
      recommendationText.innerHTML = data.error?.message || "Unknown error.";
      return;
    }
    ...
    result.split("\n").forEach(line => {
      if (line.startsWith("Risk Level:")) {
        risk = line.replace("Risk Level:", "").trim();
      } else if (line.startsWith("Indicators:")) {
        indicators = line.replace("Indicators:", "").trim();
